...
//   alt={i18n.language === "ar" ? grade.name_ar : grade.name}
//   className="h-32 w-full sm:h-full object-cover group-hover:scale-110 group-hover:opacity-30 transition duration-500 ease-in-out"
//   style={{ aspectRatio: '3/2' }} // Ensures consistent aspect ratio
// />
...
export default function ChooseSchool() {
                  ...
                  <div className="relative overflow-hidden mt-2 lg:mt-4 rounded-3xl group">
                    <Link to={`/school/${item.name}/grade/${grade.id}`}>
                      <img 
                        ...
                        className="h-32 w-full sm:h-full object-cover group-hover:scale-110 group-hover:opacity-30 transition duration-500 ease-in-out"
                      />
                      <h4 className="absolute bottom-[5%] left-[5%] whitespace-nowrap text-sm sm:text-xl xl:text-2xl font-bold text-white group-hover:bottom-1/2 group-hover:translate-y-1/2 group-hover:left-1/2 group-hover:-translate-x-1/2 duration-300 transition-all">
                        {i18n.language === "ar" ? grade.name_ar : grade.name}
                      </h4>
                    </Link>
                  </div>
  ...
  {data ? (
    data.data.map((item) => (
      <div
        key={item.id}
        className="col-span-6 lg:col-span-12 grid grid-cols-6 lg:grid-cols-12 gap-2 sm:gap-4 mb-10"
      >
        <div className="col-span-6 lg:col-span-12">
          <h4 className="lg:text-xl xl:text-3xl font-bold text-white bg-[#1E2A4A] border border-primary rounded-full inline-block py-2 px-4">
            {i18n.language === "ar" ? item.name_ar : item.name}
          ...
          <div key={grade.id} className="col-span-3 lg:col-span-4 rounded-2xl">
            <div className="relative overflow-hidden mt-2 lg:mt-4 rounded-3xl group">
              <Link to={`/school/${item.name}/grade/${grade.id}`}>
                ...
                <h4 className="absolute bottom-5 left-5 text-nowrap text-sm sm:text-xl xl:text-2xl font-bold text-white 
                               group-hover:bottom-1/2 group-hover:translate-y-1/2 group-hover:left-1/2 group-hover:-translate-x-1/2 
