...
//         {ReversedLesson?.map((lesson, index) => {
//           return(
//           <NavLink
//             key={lesson.id}
//             to={`lesson/${lesson.id}`}
...
const LessonsPaginator = () => {
  const { state: lessonsNum } = useLocation();
  const { t, i18n } = useTranslation();
  const { lessonID } = useParams();  // Get the current lesson ID from the URL
    ...
    <div className="rounded-lg p-2 lg:p-6 w-full mb-10 hover:shadow-[6px_6px_0px_0px_#F15C54] duration-150 transition-all shadow-[4px_4px_0px_0px_#F15C54]">
      ...
      <div className="space-y-4">
        {lessonsNum?.map((lesson, index) => {
          const isActive = lesson.id === lessonID;  // Check if the lesson is active
          return (
            <Link
              key={lesson.id}
              to={`lesson/${lesson.id}`}
              state={lessonsNum}
              className={`flex items-center gap-2 bg-slate-900 rounded-lg px-2 lg:px-4 py-2 text-white hover:bg-slate-700 flex-wrap ${isActive ? 'active' : ''}`} // Apply active class
            >
              <p className="text-base">{i18n.language === 'ar' ? " الدرس ال" : " Lesson"} {index + 1} : </p>
              <p className="text-sm">{lesson.title}</p>
            </Link>
