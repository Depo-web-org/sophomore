  ...
  return  <>
    ...
    <div className="grid grid-cols-1   gap-8 w-full ">
      {statisticsData.filter(i=> i.title != 'Completed Tasks').map((item, index) => (
        <StatisticCard
          key={index}
          style={"flex justify-center md:justify-start  rounded-xl items-center gap-8 bg-white p-8 group hover:shadow-lg rounded-md"}
          image={item.image}
          title={item.title}
          stats={item.stats}
        />
...
function UploadCourse(props) {
  return (
    <Link to="addnewcourse" className="bg-primary rounded-md px-4 py-1  lg:px-8 lg:py-2  hover:bg-secondary transition-all duration-300   font-semibold text-white text-center text-sm lg:text-base">
      {props.Text}
    </Link>
  );
}
