...
<div  className="relative " >
      ...
      <div className="absolute top-10 md:top-0 left-1/2 transform -translate-x-1/2 w-full container   ">
        <div className="text-white w-96 mx-auto text-center font-semibold">
          ...
          <p className="text-2xl sm:text-3xl">You got {result.totalCorrect}/{result.totalQuestions} Questions</p>
        </div>
          ...
          <div>

            {
              result?.result.map((answers)=>{ 
                return  <div key={answers.answer}  className="flex justify-items-center items-center">
                  {
                    answers.result  ?  <AiFillCheckCircle className="text-green-400 w-6 h-6 my-2 me-1" />  :      <AiFillCloseCircle className="text-red-500 w-6 h-6 my-2 me-1" />
                  }
                -
                 <p className="mx-1"> {answers.answer}</p>   <p className="mx-1 text-green-400"> {answers.trueAnswer}</p> 
              </div>
              })
            } 
          </div>
